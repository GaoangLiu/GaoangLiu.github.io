...
笔者在线下使用 ChatGPT(version: 2023.12.05) 做了一下实验，以上面的*理想气体状态*任务为例，让 ChatGPT 直接回答原问题，得到的答案是“气体压力会增加 2 倍”，这与正确答案不符合。
```text
... 
...
而使用 Step-Back prompting 的方式，ChatGPT 在 factor 数值（`4`）上推理有提升，但在最后一步把因子放错了位置（对应于文中的 *Math Error*），进而在最后推理时也出现错误。正确结果应该是 $P_1 = 4P_2$。
...
从研究人员在 TimeQA 上的实验结果中可以看到，在简单任务上，RAG, Step-Back 都能得到不少的提升。对于困难任务，RAG 提升不明显，而 Step-Back（不带 RAG）的提升仍然比较明显。作者没有给出具体的结果差异，但笔者猜测，Step-Back 通过 LLM 检索得到更高层（全面）的知识，而 RAG 检索获得的还是偏细节的内容，从而缺少推理准确答案需要的额外信息。 
