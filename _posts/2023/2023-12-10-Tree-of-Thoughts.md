...
[Tree of thoughts(ToT)](https://arxiv.org/abs/2305.10601) 是由 Princeton 及 Google DeepMind 提出的一种新的模型推理框架，是 [Chain of Thoughts Prompting](https://arxiv.org/abs/2201.11903)(CoT) 的泛化形态。ToT 将语言模型的推理建模成对树的搜索过程，通过将问题逐步拆解成更易处理的子问题，在每一步探索并缓存可行的解题路径，从而提高 LM 解决问题的能力。论文： [Tree of Thoughts: Deliberate Problem Solving with Large Language Models](https://arxiv.org/abs/2305.10601)。
...
CoT 的提出是为了解决输入 $$x$$ 到输出 $$y$$ 的映射非平凡（non-trivial）的情况，即难以通过简单的一步推理就可以解决的问题，典型代表是数学、逻辑问题。CoT 背后的思想是从 $$x$$ 到生成 $$y$$ 的过程中，引入一系列 thoughts $$z_i$$ 来帮助模型进行推理。对数学题来说，$$z_i$$ 可以是解题时的中间步骤。 形式化地，CoT: $$y \sim p_\theta^\text{CoT}(x, z_{1,...,n})$$，其中:
...
CoT-SC（Self-consistency with CoT）是一种集成策略，通过考虑并综合多个 i.i.d 的 CoT 的结果做最终的决策。形式化表示为：$$[z_{1, ...,n}^{(i)}, y^{(i)}] \sim p_\theta^\text{CoT}(z_{1,...,n}, y \vert x)$$，返回的出现频率最高的结果 $$y=\argmax_y \sum_i \mathbb{I}(y^{(i)}=y)$$。CoT-SC 比朴素的 CoT 有效的原因是，同一个问题通常存在不同的解题过程，如果可以探索更多的解题路径，并对最终结果进行投票表决，输出决策可能更加可靠。这种方式比较适用于输出结果有限的情况，比如多选问答。 

CoT-SC 存在的一个问题是，它探索的这些路径除了起点一致、在终点参与决策以外，彼此之前是独立的、没有关联的。 

人类在解决问题会采用的一种思路是，将问题拆成若干个步，每一步都考察多个解决方案，这些过程与方案构成一个树状结构，解决问题相当于在这棵树上进行搜索，逐渐找到一条可行的解决路径。搜索过程中，选择哪个分支可以借助一些启发式策略。ToT 的思想让 LMs 解决问题时也参照类似的思路，从而解决 LMs 的两个关键的不足：
1. 局部的，LMs 在解决问题时“一条路走到黑”，不会思考探索不同的可能性；
2. 全局的，LMs 缺乏整体规划(planning)、预测(lookahead)及回溯(backtracking)，进而无法辅助评估不同的决策。
...
*Thought* 指的就是解题过程中 LM 生成的中间结果，这个结果用于辅助 LM 做后续的思考与推理。 
...
Thought 的设计需要足够“小”，以便语言模型生成有前景且多样的样本，但又需要足够“大”，以便语言模型能够评估这个 thought 在问题解决方面是否有前景。

## 思维生成 / Thoughts Generation
给定一个状态 $$s=[x, z_{1,...,i}]$$，ToT 提出了两种用于生成下一步 $$k$$ 个 thoughts 的策略：
1. 从 CoT prompting 结果中独立同分布采样，$$z^{(j)} \sim p_\theta^\text{CoT}(z_{i+1} \vert s)$$。
2. 通过 "propose prompt" 生成 thoughts $$[z^{(1)}, ..., z^{(k)}] \sim p_\theta^\text{propose}(z_{i+1}^{(1..k)} \vert s)$$。😕

## 状态评估 $$V(p_\theta, S)$$ <span id="state-evaluate"></span>
通过 LLM + few shots 启发式的选择可行状态，启发式方法比规则硬编码更灵活，也不需要专门训练一个模型。与思维生成类似，研究人员也设计了两种策略：
...
2. 通过不同状态的投票：$$V(p_\theta, S)(s) \sim \mathbb{I}[s = s^*]$$，其中 $$s^* = \argmax_s \sum_i \mathbb{I}[s^{(i)}=s]$$。适用场景，判断段落连续性。

这里面有两个概念，分别是 state 与 thought，那么二者有什么关系 ？如上所述，thought 是指导 LLM 一步步去解决问题的中间步骤/结果，可以是一条公式，或者一条写作计划，在论文里 thought 用 $$z_i$$ 表示。而 state 指的是由 input 及一系列 thoughts 构成的解题路径，在论文里用 $$[x, z_{1,...,i}]$$ 表示。state 的评估是为了判断当前 thought 是否可行，是否有前景，是否应该继续探索下去。


### 思维生成与状态评估决定最终的效果
ToT 最重要的两步无疑是**思维生成与状态评估**，任何一个效果不佳，对模型解题都会带来困难，对于需要精细推理（deliberate reasoning）的问题来说更是如此。通俗的讲，思维生成和搜索算法用于探索可行的解题路径，而状态评估用于鉴别哪些路径是可行的。
...
差距较大的一个原因可能是笔者在实验中使用了 ChatGPT 而非 GPT-4，效果上有折扣。如上面[状态评估](#state-evaluate)小节里分析，ChatGPT 在<span style="color:red">状态评估阶段效果并不理想</span>。针对这一点，笔者还专门做了一个小实验，这个实验的任务跟 state evaluation 一致，即给定第一次思维生成后的状态（e.g., `3 4 5 7\n7-5=2(left: 3 4 2)`），让 ChatGPT 判断凑成 24 的可能性是 `sure`、`likely` 还是 `impossible`，实验数据是 30 条结果都应该为 `sure` 的状态，数据集见[thoughts.txt](https://dlj.one/h9m6gl)。实验重复 3 次并求平均结果，ChatGPT 在 30 条状态评估的正确率仅为 0.17，也即是错误率在 0.8 以上。
...
研究人员还统计了 CoT，ToT 失败步数比例，失败指走到某一步无法继续推理下去，比如所有 thoughts 都是 invalid。如下图 Figure 3(a) 所示，CoT 有超过 60% 的比率在第一步就错了，ToT($$b=5$$) 的失败情况都集中在最后一步，前面几步很少有出错的。对于基于树的搜索算法来说，这是结果并没有让人意外的地方，当 $$b=5$$ 时 ToT 理论上可能考虑 $$5^4=625$$ 种可能的路径，而 CoT 只生成一条路径，因此半道折戟的概率会更高。
...
# 代价
ToT 对比 CoT 在效果上确实有提升，但对应的计算量的增加也是显而易见的，根据 prompt 的设计及搜索算法的不同，ToT 生成的 tokens 数量通常是是 CoT 的 5 - 100 倍。Game of 24 及 Creative Writing 的实验花费了 106 刀。在 Game of 24 上平均每个 case 大概消耗 6.9k 个 tokens。因此研究人员也推荐，仅当任务需要精细推理且 CoT 效果不好时，才使用 ToT。
